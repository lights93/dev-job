...
public class CrawlKakaoService implements CrawlService {
	...
	public Flux<Recruit> crawl() {
		...
		String trim = document.select(".link_job.link_job1").text().trim();

		Matcher countMatcher = NUMBER_PART_PATTERN.matcher(trim);
		int pages = 0;

		if (countMatcher.find()) {
			pages = (Integer.parseInt(countMatcher.group(0)) + 10 - 1) / 10;
		}
		...
		return Flux.range(1, pages)
			.map(i -> {
				Document doc = null;
				try {
					doc = Jsoup.connect(KAKAO_RECRUIT_URL + "/jobs")
						.followRedirects(false)
						.method(Connection.Method.GET)
						.data("part", "TECHNOLOGY")
					...
					log.error("kakao page parse error , page={}", i, e);
				}
